...
def generate_description(row):
    ...
    if row['region'] == 'whole body' and pd.notnull(row['M-stage']):
        if row['M-stage'] == 'M0':
    ...
    # TNM group 설명 (whole body인 경우에만)
    if row['region'] == 'whole body' and pd.notnull(row['TNM group']):
        if row['TNM group'] == 'I':
...
# JSON 파일로 저장
output_json_path = '/home/hb0522/MLLM/MLLM2/Description/Descriptions_hector3D.json'
