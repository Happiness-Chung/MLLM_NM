...
class ModelArguments:
    ...
    model_name_or_path: Optional[str] = field(default="liuhaotian/llava-v1.6-vicuna-7b")
    ...
    vision_tower: Optional[str] = field(default="openai/clip-vit-base-patch32")
...
def _add_speaker_and_signal(header, source, get_conversation=True):
    ...
    for sentence in source:
        ...
        # sentence["value"] = BEGIN_SIGNAL + sentence["value"] + END_SIGNAL
        if get_conversation:
...
def preprocess_test(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    conv = conversation_lib.default_conversation.copy()
    conv.system = ""
    ...
    for i, source in enumerate(sources):
        ...
        # For input (questions only)
        input_conv = conv.copy()
        input_conv.messages = []
        
        # For target (full conversation)
        target_conv = conv.copy()
        target_conv.messages = []
        
        for j, sentence in enumerate(source):
            role = roles[sentence["from"]]
            target_conv.append_message(role, sentence["value"])
            if role == conv.roles[0]:  # Only add human messages to input conversation
                input_conv.append_message(role, sentence["value"])

        input_conversations.append(input_conv.get_prompt())
        target_conversations.append(target_conv.get_prompt())

    # Tokenization for input conversations
    if has_image:
        input_ids = torch.stack([tokenizer_image_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors="pt") for prompt in input_conversations], dim=0)
    else:
        input_ids = tokenizer(
            input_conversations,
            return_tensors="pt",
    ...
    # Tokenization for target conversations
    targets = tokenizer(
        target_conversations,
        return_tensors="pt",
        padding="longest",
    ...
    # Mask targets
    sep = conv.sep + conv.roles[1] + ": "
    for conversation, target in zip(target_conversations, targets):
        total_len = int(target.ne(tokenizer.pad_token_id).sum())
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    if conversation_lib.default_conversation.version.startswith("v1"):
        # print("##################################v1")
        # return preprocess_test(sources, tokenizer, has_image=has_image)
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        try:
            ...
            # print(f"Loaded image: {image_file} with size {image.size}")  # 디버깅 메시지
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        # print("Source o: ", sources[0])
        if "image" in sources[0]:
            image_file = self.list_data_dict[i]["image"]
            if type(image_file) is list:
                # print("This part is executed. !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                image = [self.process_image(f) for f in image_file]
                # print(f"Processed image {image_file} for sample {i}")  # 디버깅 메시지
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            ...
            # print("################################ image sizes", batch["image_sizes"])
            batch["modalities"] = [im[2] for im_list in images for im in im_list]
            # print("################################", batch["modalities"])
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        # config를 불러올 때 AutoConfig를 사용하여 최신 설정을 가져옵니다.
        config = LlamaConfig.from_pretrained(model_args.model_name_or_path)

        if 'config' in customized_kwargs:
            del customized_kwargs['config']
...
def train(attn_implementation=None):
    ...
    ### login
    login(token="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")
    ...
    # Sampling based generation : increases diversity and uncertainty. 
    # In medical, it is unnecessary. We remove below features.
    ...
    try:
        ...
        print(f"Generation config validation error: {e}")
