...
class Trainer:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    ...
                    def convert_to_half(tensor):
                        if isinstance(tensor, torch.Tensor) and tensor.dtype == torch.float32:
                            return tensor.half()
                        elif isinstance(tensor, list):
                            # return [convert_to_half(item) for item in tensor]
                            pass
                        else:
                            return tensor

                    # inputs = {k: convert_to_half(v) for k, v in inputs.items()}
                    inputs = {k: convert_to_half(v) if k not in ["image_sizes", "modalities"] else v for k, v in inputs.items()}
                    # print(len(inputs['image']))

                    # 1. NaN 또는 Inf 값 확인
                    # print(inputs['input_ids'])
                    nan_or_inf = torch.isnan(inputs['input_ids']) | torch.isinf(inputs['input_ids'])

                    # 2. 소수점 값을 가지는지 확인
                    not_integer = inputs['input_ids'] != inputs['input_ids'].floor()

                    # 3. 0보다 작거나 vocab_size 이상인 값 확인
                    # invalid_range = (inputs['input_ids'] < 0) | (inputs['input_ids'] >= 32000)

                    # 4. 종합적인 invalid_ids 텐서 생성
                    # torch.isfinite는 NaN, Inf, -Inf 모두를 처리
                    invalid_ids = nan_or_inf | not_integer # | invalid_range

                    # # invalid_ids 텐서에서 True 값의 인덱스 찾기
                    invalid_indices = torch.nonzero(invalid_ids)
                    # print(invalid_indices)

                    # 5. 문제가 있는 값을 0으로 대체 (또는 적절한 값으로 설정)
                    inputs['input_ids'][invalid_indices] = 0  # 0 또는 다른 적절한 값으로 대체

                    # if invalid_indices.numel() > 0:
                    #     print(f"Invalid token IDs found at indices: {invalid_indices}")
                    #     print(inputs['input_ids'][invalid_indices[0][0]][invalid_indices[0][1]])
                    # else:
                    #     print("All token IDs are within the valid range.")
                    # ######################################################################################
                    tr_loss_step , outputs, labels = self.training_step(model, inputs)
                    # print(tr_loss_step.dtype)
                    # ######################################################################################
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        # loss말고 generated text도 output으로 받아도 될듯
    ...
    def compute_loss(self, model, inputs, return_outputs=False, test = False):
        """
        ...
        if test == True:
            with torch.no_grad():
                outputs, labels = model(**inputs, labels = labels) ### teacher forcing
        else:
                outputs, labels = model(**inputs, labels = labels) ### teacher forcing
        ...
        # 마스킹을 사용하여 -100이 아닌 위치의 predicted_token_ids만 선택
        # mask = labels != -100
        # predicted_token_ids = torch.masked_select(predicted_token_ids, mask)
        filtered_predicted_token_ids = predicted_token_ids[0][labels[0] != -100]
        filtered_labels0 = labels[0][labels[0] != -100]
        
        # 1. NaN 또는 Inf 값 확인
        # nan_or_inf = torch.isnan(predicted_token_ids) | torch.isinf(predicted_token_ids)

        # # 2. 소수점 값을 가지는지 확인
        # not_integer = predicted_token_ids != predicted_token_ids.floor()

        # # 3. 0보다 작거나 vocab_size 이상인 값 확인
        # invalid_range = (predicted_token_ids < 0) | (predicted_token_ids >= 32000)
        ...
        # # 5. 문제가 있는 값을 0으로 대체 (또는 적절한 값으로 설정)
        # predicted_token_ids[invalid_indices] = 0  # 0 또는 다른 적절한 값으로 대체
        # print(predicted_token_ids[invalid_indices])

        # 1. NaN 또는 Inf 값 확인
        nan_or_inf = torch.isnan(filtered_labels0) | torch.isinf(filtered_labels0)

        # 2. 소수점 값을 가지는지 확인
        not_integer = filtered_labels0 != filtered_labels0.floor()

        # 3. 0보다 작거나 vocab_size 이상인 값 확인
        invalid_range = (filtered_labels0 < 0) | (filtered_labels0 >= 32000)
        ...
        # 5. 문제가 있는 값을 0으로 대체 (또는 적절한 값으로 설정)
        filtered_labels0[invalid_indices] = 0  # 0 또는 다른 적절한 값으로 대체
        # print(filtered_labels0[invalid_indices])
        print(f"Length of input_ids: {inputs['input_ids'].shape[1]}")
        print(f"Generated tokens: {len(filtered_predicted_token_ids)}")
        print(f"Label tokens: {len(filtered_labels0)}")
        generated_text0 = tokenizer.decode(filtered_predicted_token_ids, skip_special_tokens=True)
        print("Generated Text:", generated_text0)
        label_text0 = tokenizer.decode(filtered_labels0, skip_special_tokens=True)
        print("Label text: ", label_text0)
        ###############################################################################
        ...
        if labels is not None:
            ...
            if model_name in MODEL_FOR_CAUSAL_LM_MAPPING_NAMES.values():
                ...
                # logits_length = outputs.logits.size(-2)
                # labels_length = labels.size(-1)

                # length_penalty = abs(logits_length - labels_length) * 0.3  # penalty_weight는 적절히 조정 가능

                # if logits_length > labels_length:
                #     # logits이 labels보다 길 경우, 크롭(crop)합니다.
                #     outputs.logits = outputs.logits[..., labels_length, :]
                # elif logits_length < labels_length:
                #     # logits이 labels보다 짧을 경우, logits을 패딩(pad)합니다.
                #     padding_size = labels_length - logits_length
                #     pad = torch.zeros(outputs.logits.size(0), padding_size, outputs.logits.size(-1), device=outputs.logits.device)
                #     outputs.logits = torch.cat([outputs.logits, pad], dim=-2)

                # 처리 후 shape를 다시 확인합니다.
                # print(outputs.logits.shape, labels.shape)
                loss = self.label_smoother(outputs, labels) # + length_penalty
        else:
    ...
    def predict(self, test_dataset: Dataset, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = "test"
        ...
        # test_dataloader = self.get_train_dataloader()
        start_time = time.time()
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels or loss_without_labels:
                    # print("####################################3")
                    with self.compute_loss_context_manager():
                        loss, outputs, labels = self.compute_loss(model, inputs, return_outputs=True, test = True) ############# 여기서 라벨도 받아와야됨
    ...
    def clean(self,inputs):
        ...
        nan_or_inf = torch.isnan(inputs) | torch.isinf(inputs)
        not_integer = inputs != inputs.floor()
        invalid_range = (inputs < 0) | (inputs >= 32000)
        invalid_ids = nan_or_inf | not_integer | invalid_range
        invalid_indices = torch.nonzero(invalid_ids)
        inputs[invalid_indices] = 0  
    ...
    def prediction_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            filtered_labels = []
            filtered_predicted_token_ids = []
            generated_texts = []
            label_texts = []
            
            for i in range(len(labels)):
                filtered_labels.append(self.clean(labels[i][labels[i] != 100])) 
                filtered_predicted_token_ids.append(self.clean(predicted_token_ids[i][labels[i] != -100]))
            
            for i in range(len(filtered_labels)):
                label_texts.append(tokenizer.decode(filtered_labels[i], skip_special_tokens=True))
                generated_texts.append(tokenizer.decode(filtered_predicted_token_ids[i], skip_special_tokens=True))

            with open(preds_file, 'a') as f:
                for i in range(len(generated_texts)):
                    f.write(f"{generated_texts[i]}\n\n\n")
            with open(labels_file, 'a') as f:
                for i in range(len(label_texts)):
                    f.write(f"{label_texts[i]}\n\n\n")
